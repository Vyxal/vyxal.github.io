...
<template>
  <div class="w-full bg-neutral-900 p-5 flex flex-wrap items-center gap-3.5 md:gap-7">
    ...
    <button class="play" title="Run">
      <svg xmlns="http://www.w3.org/2000/svg" @click="cancel" class="cog" viewBox="0 0 24 24" stroke-width="1.5"
        stroke="white" v-if="worker">
    ...
    <button class="!mt-auto" @click="modalIsOpen = true" v-if="closedTabs.length" title="Open a new tab">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <path fill-rule="evenodd"
    ...
    outputLink(type: string) {
      ...
      switch (type) {
        case "permalink":
          ...
          break;
        case "cmc":
          store.output = `[Vyxal 3, ${len} byte${code.length != 1 ? "s" : ""}${utfable ? "" : " (UTF-8)"
            }: \`${code.replace(/`/g, "\\`")}\`](${link})`;
          break;
        case "cgcc":
          if (flags.includes("l")) {
          ...
          store.output = `# [Vyxal 3](https://github.com/Vyxal/Vyxal/tree/version-3)${flagAppendage}, ${len} byte${len != 1 ? "s" : ""
            }${utfable ? "" : " (UTF-8)"}
\`\`\`
${code}
\`\`\`
[Try it online!${flags.includes("l") ? " (link is to literate version)" : ""
            }](${link})${code.includes("🍪")
              ? "\n\n🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪🍪"
              : ""
            }`;
