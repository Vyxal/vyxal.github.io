        ...
        <Modal.Body className="vstack">
            {inputs.map((input) => {
                return <InputGroup className="mb-2 p-1" key={input.id}>
                    <Form.Control as="textarea" placeholder="Input" value={input.value} onChange={(event) => setInputs(inputs.map((item) => item.id == input.id ? { id: item.id, value: event.currentTarget.value } : item))} />
